...
SELECT a.nombre AS 'asignatura', ce.anyo_inicio AS 'any inici', ce.anyo_fin AS 'any fi', p.nif AS 'nif'
FROM persona p
...
SELECT d.nombre AS 'departamento', COUNT(pr.id_departamento) AS 'profesores'
FROM departamento d
JOIN profesor pr
	ON pr.id_departamento = d.id
GROUP BY d.nombre
...
SELECT d.nombre AS 'departamento', COUNT(pr.id_profesor) AS 'profesores'
FROM departamento d
LEFT JOIN profesor pr
	ON pr.id_departamento = d.id
GROUP BY d.nombre
...
-- Tingui en compte que poden existir graus que no tenen assignatures associades. Aquests graus també han d'aparèixer en el llistat. El resultat haurà d'estar ordenat de major a menor pel nombre d'assignatures.
...
SELECT g.nombre AS 'grado', a.tipo, SUM(a.creditos) AS 'creditos'
FROM grado g
JOIN asignatura a
	ON a.id_grado = g.id
...
SELECT ce.anyo_inicio, COUNT(asma.id_alumno) AS 'alumnos matriculados'
...
SELECT p.id, p.nombre, p.apellido1, p.apellido2, COUNT(a.id) AS 'nombre_asignaturas'
FROM persona p
LEFT JOIN asignatura a
	ON a.id_profesor = p.id
WHERE p.tipo = 'profesor'
GROUP BY p.id
ORDER BY nombre_asignaturas DESC;
...
WHERE pr.id_profesor NOT IN
    ...
    FROM asignatura a
    WHERE a.id_profesor IS NOT NULL);
