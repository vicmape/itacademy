...
SELECT nombre, precio AS 'precio EUR', ROUND((precio * 1.08),2) AS 'precio USD' FROM producto;
...
SELECT f.nombre AS 'fabricante', p.nombre AS 'producto', p.precio AS 'precio' FROM producto p JOIN fabricante f ON p.codigo_fabricante = f.codigo ORDER BY f.nombre ASC;
...
SELECT p.nombre AS 'nombre producto', p.precio, f.nombre AS 'nombre fabricante' FROM producto p JOIN fabricante f ON p.codigo_fabricante = f.codigo ORDER BY p.precio ASC LIMIT 1;
...
SELECT p.nombre AS 'producto' , f.nombre AS 'fabricante', p.precio FROM producto p JOIN fabricante f ON p.codigo_fabricante = f.codigo WHERE f.nombre = 'Crucial' AND p.precio > 200;
...
SELECT p.nombre AS 'producto', f.nombre AS 'fabricante' FROM producto p JOIN fabricante f ON f.codigo = p.codigo_fabricante WHERE f.nombre = 'Asus' OR f.nombre = 'Hewlett-Packard' OR f.nombre = 'Seagate';

 -- 29 - Retorna un llistat amb tots els productes dels fabricants Asus, Hewlett-Packard i Seagate. Utilitzant l'operador IN.
SELECT p.nombre AS 'producto', f.nombre AS 'fabricante' FROM producto p JOIN fabricante f ON f.codigo = p.codigo_fabricante WHERE f.nombre IN('Asus', 'Hewlett-Packard', 'Seagate');

-- 30 - Retorna un llistat amb el nom i el preu de tots els productes dels fabricants el nom dels quals acabi per la vocal e.
SELECT p.nombre AS 'producto', p.precio AS 'precio', f.nombre AS 'fabricante' FROM producto p JOIN fabricante f ON f.codigo = p.codigo_fabricante WHERE f.nombre LIKE '%e';

-- 31 - Retorna un llistat amb el nom i el preu de tots els productes el nom de fabricant dels quals contingui el caràcter w en el seu nom.
SELECT p.nombre AS 'producto', p.precio AS 'precio', f.nombre AS 'fabricante' FROM producto p JOIN fabricante f ON f.codigo = p.codigo_fabricante WHERE f.nombre LIKE '%w%';
...
SELECT p.nombre AS 'producto', p.precio AS 'precio', f.nombre AS 'fabricante' FROM producto p JOIN fabricante f ON f.codigo = p.codigo_fabricante WHERE p.precio >= 180 ORDER BY p.precio DESC, p.nombre ASC;

-- 33 - Retorna un llistat amb el codi i el nom de fabricant, solament d'aquells fabricants que tenen productes associats en la base de dades.
SELECT DISTINCT f.codigo AS 'codigo fabricante', f.nombre AS 'fabricante' FROM fabricante f JOIN producto p ON f.codigo = p.codigo_fabricante;
...
SELECT f.nombre AS 'fabricante' FROM fabricante f WHERE f.codigo NOT IN (SELECT f.codigo FROM fabricante f JOIN producto p ON p.codigo_fabricante = f.codigo);
...
SELECT p.nombre AS 'producto caro Lenovo', p.precio AS 'precio' FROM producto p JOIN fabricante f ON p.codigo_fabricante = f.codigo WHERE f.nombre = 'Lenovo' ORDER BY p.precio DESC LIMIT 1;

-- 39 - Llista el nom del producte més barat del fabricant Hewlett-Packard.
SELECT p.nombre AS 'producto barato HP', p.precio AS 'precio' FROM producto p JOIN fabricante f ON p.codigo_fabricante = f.codigo WHERE f.nombre = 'Hewlett-Packard' ORDER BY p.precio ASC LIMIT 1;
...
SELECT p.nombre AS 'productos' FROM producto p WHERE p.precio >= (SELECT p.precio FROM producto p JOIN fabricante f ON p.codigo_fabricante = f.codigo WHERE f.nombre = 'Lenovo' ORDER BY p.precio DESC LIMIT 1);

-- 41 - Llesta tots els productes del fabricador Asus que tenen un preu superior al preu mitjà de tots els seus productes.
SELECT p.nombre AS 'producto', p.precio AS 'precio', f.nombre AS 'fabricante' FROM producto p JOIN fabricante f ON p.codigo_fabricante = f.codigo WHERE f.nombre = 'Asus' AND p.precio > (SELECT AVG(p.precio) FROM producto p JOIN fabricante f ON p.codigo_fabricante = f.codigo WHERE f.nombre = 'Asus');
